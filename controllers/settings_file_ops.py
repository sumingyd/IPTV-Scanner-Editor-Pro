...
class SettingsFileOperations:
    ...
    def player_settings(self):
        ...
        # 加载现有数据到UI（load_subscription_sources_to_ui 内部会自动查找 dialog 中的 QListWidget）
        if hasattr(self.window, '_load_subscription_sources_to_ui'):
            self.window._load_subscription_sources_to_ui()
    ...
    def set_language(self, language: str):
        ...
        if hasattr(self.window, 'language_manager'):
            ...
            # 更新UI文本
            if hasattr(self.window, '_update_ui_text'):
                self.window._update_ui_text()
